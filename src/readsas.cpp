...
Rcpp::List readsas(const char * filePath, const bool debug, const uint64_t kk)
{
  ...
  if (sas) {
    ...
    for (auto pg = 0; pg < pagecount; ++pg) {
            ...
            PAGE_TYPE == 128 || PAGE_TYPE == 0 ))
      {
          ...
          case 1:
            {
              ...
              if (u64 == 4) {
                ...
                if (debug) Rcout << rowsonpg << std::endl;


                unk16 = readbin(unk16, sas, swapit); // 1
                if (unk16 != 0) stop("unk16 01. expected 0 is %d", unk16);
                unk32 = readbin(unk32, sas, swapit); // 2
                if (debug) Rcout << "2 " << unk32 << " " <<
                  sas.tellg() << std::endl;
                unk16 = readbin(unk16, sas, swapit); // 4
                ...
                for (int z = 0; z < 8; ++z) {
                  unk64 = readbin((int32_t)unk64, sas, swapit);
                  if (unk64 != 0) warning("unk64 %d: expected 0 is %d",
                      z, unk64);
                }
                ...
                for (int z = 0; z < 3; ++z) {
                  unk32 = readbin(unk32, sas, swapit); // 0
                  // if (unk32 != 0) warning("unk32 %d: expected 0 is %d",
                  //     z, unk32);
                }
          ...
          case 5:
            {
              ...
              // Rcout << len << std::endl;
              guess = readbin(guess, sas, swapit); // 0 | 16 maxlen?
              // if (guess != 0) stop("guess 1. expected 0 is %d", guess);
              unk16 = readbin(unk16, sas, swapit); // 0 | 128
          ...
          case 9:
            {
              ...
              /* not sure about the purpose. appears in a few SAS 9 files */
               // point to observation?
              uint64_t unklen = potabs[sc].SH_LEN - alignval;
              if (debug) Rcout << "c9; "<< unklen << std::endl;
        ...
        if (PAGE_TYPE == 384) {
          ...
          // pos 128456 in the test.sas7bdat file defines if values are dropped
          // initial 00 = all is read
          // 60 = keep only the first
          // 40 = drop the second
          // 90 = drop the first
          // still this position is 980 from sas_offset_table == 3 how does sas
          // end up there?

          // // ref from parso
          // // sas.seekg(12, sas.cur);
          // unk32 = readbin(unk32, sas, swapit);
          // Rcout << "unk32: " << unkdub << std::endl;
          //
          // sas.seekg(pagepos, sas.beg);
        ...
        // if (PAGE_TYPE == 640) {
          ...
          for (int ppp = 0; ppp <= pg; ++ppp)
            Rcout << "data_pos[ppp]: " << data_pos[ppp] << std::endl;

          // position on page pg at end of page or beginning of subheader
          auto pagepos = (headersize + (pg+1) * pagesize);
          pagepos -= 24;
          sas.seekg(pagepos, sas.beg);
        ...
        // }
        if (PAGE_TYPE == -28672) rowcount -= delobs;
    ...
    // if (i >= 129 & i <= 133)
        Rcout << "Row "<< i << " at: " << sas.tellg() << std::endl;
